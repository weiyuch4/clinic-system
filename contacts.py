...
from models import FollowupEntry, MsptStage, MsptSubmittableEntry, Patient
...
def init() -> None:
    with _conn() as conn:
        ...
        # Migration: add last_visit_date to existing databases
        try:
            conn.execute("ALTER TABLE contacts ADD COLUMN last_visit_date TEXT")
...
def get_contacted_with_dates() -> list[tuple[FollowupEntry, date]]:
    ...
    return [
        (_rows_to_followup_entries([r[:10]])[0], date.fromisoformat(r[10]))
        for r in rows
    ]
...
def get_called_entries() -> list[FollowupEntry]:
    """Entries permanently hidden after second notification (attempt=2)."""
    with _conn() as conn:
        rows = conn.execute(
            """SELECT chart_number, name, birth_date, disease_name, category, due_date,
                      days_overdue, mspt_stage, contact_reason, last_visit_date, contacted_at
               FROM contacts WHERE attempt=2""",
        ).fetchall()
    return [
        _rows_to_followup_entries([r[:10]])[0].model_copy(
            update={"contacted_at": date.fromisoformat(r[10]) if r[10] else None}
        )
        for r in rows
    ]
