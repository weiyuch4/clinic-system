...
(function () {
  ...
  // Decode role from JWT payload (base64url, no signature needed client-side)
  var _role = (function () {
    if (!_token) return '';
    try {
      var seg = _token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/');
      return JSON.parse(atob(seg)).role || '';
    } catch (e) { return ''; }
  }());
  ...
  var ICONS = {
    ...
    prescription: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="8" width="20" height="8" rx="4"/><line x1="12" y1="8" x2="12" y2="16"/></svg>',
    ...
    admin:     '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>',
  };
  ...
  var MAIN_NAV = [
    ...
    { id: 'chronic',   label: '慢簽追蹤',  href: '/chronic',     badge: true,  hidden: true },
  ...
  function _renderSidebar() {
    ...
    el.innerHTML =
      ...
      '<div class="sb-nav">' +
        ...
        MAIN_NAV.filter(function(i){ return !i.hidden; }).map(_navItem).join('') +
        '<div class="nl">其他</div>' +
      ...
      '<div class="sb-bot">' +
        ...
        '</div>' +
        (_role === 'admin' ?
          '<button class="adm-entry" type="button" onclick="window.open(\'/admin\',\'_blank\',\'noopener\')">' +
